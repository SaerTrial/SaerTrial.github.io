...
I made some changes on ange to run this firmware via pcode. I found mismatches at first when comparing registers in angr and those in `sim_avr`. I ended up realizing a faulty pcode implementation on logical shift instructions, which behave same as arithemtic shift. In order to solve this issue, I change `make_bv_sizes_equal` in `angr/engines/pcode/behavior.py` file:
```
def make_bv_sizes_equal(bv1: BV, bv2: BV, signed_extend=True) -> tuple[BV, BV]:
    """
    Makes two BVs equal in length through sign extension.
    """

    if bv1.size() < bv2.size():
        return (bv1.sign_extend(bv2.size() - bv1.size()), bv2) if signed_extend else (bv1.zero_extend(bv2.size() - bv1.size()), bv2)
    elif bv1.size() > bv2.size():
        return (bv1, bv2.sign_extend(bv1.size() - bv2.size())) if signed_extend else (bv1, bv2.zero_extend(bv1.size() - bv2.size()))
    else:
        return (bv1, bv2)
```

After those changes, angr produces the same concrete register values as `simavr`. I am planning to conduct a symbolic execution, running the angr towards the solution program point and solving path constraints gathered along the way. I do have a time though and put it in my back log.

The whole python script is provided as following:
...
firmware = "/home/user/jumpy/jumpy.hex"
binary = "/home/user/jumpy/jumpy.bin"

avatar = Avatar(arch=AVR, output_directory="/tmp/jumpy")

#logging.getLogger("avatar").setLevel(logging.WARNING)
#logging.getLogger("angr").setLevel(logging.DEBUG)

simduino = avatar.add_target(SimduinoTarget,
                        simduini_executable="/home/user/simavr/examples/board_simduino/obj-x86_64-linux-gnu/simduino.elf",
                        avr_gdb_executable="avr-gdb",
                        firmware=firmware
                        )

angr_target = avatar.add_target(AngrTarget, 
                            binary_path=firmware,
                            angr_backend="hex",
                            # pcode_lang_id="avr8:LE:16:atmega256", 
                            load_options=None,
                            entry_point=0x0,
                            base_addr=0x0,
                            name="AngrTarget0",
                            simduino_target=simduino)

avatar.init_targets()

logging.getLogger(simduino.name).setLevel(logging.DEBUG)

avatar.load_plugin('gdb_memory_map_loader')

mem_ranges = simduino.load_memory_mappings(update=True)
synced_mem_ranges = []
for mem in mem_ranges.items():
    if mem.data.name == "sram":
        synced_mem_ranges.append(mem.data)

if len(synced_mem_ranges) == 0:
    raise("No mem ranges found")
...
bkt = simduino.set_breakpoint(0xd0)

cur_input = b"1234567"

simduino.cont()

print("[*] Sending input %s" % cur_input.decode())

picocom.send(cur_input + b"\r\n")

simduino.wait()

simduino.remove_breakpoint(bkt)

print( "PC2: 0x%x" % simduino.read_register("PC2"))
print( "pc: 0x%x" % simduino.read_register("pc"))

serialization = lambda data: "".join([chr(i) for i in data if i != 0])
res = serialization(simduino.rm(0x80013e, 1, 16))
print(res, " Passed: %r" % (res == cur_input.decode()))

sram_base_addr = 0x800000

sram = bytes(simduino.rm(sram_base_addr, 1, 0x900))

print("Found input located at offset 0x%x" % (sram_base_addr + sram.find(cur_input)))

simduino.set_watchpoint(0x80011b, write=False, read=True)

angr_target.avatar_state(simduino.read_register("PC2"), simduino.read_register("SP"))
avatar.transfer_state(simduino, angr_target, synced_ranges=synced_mem_ranges)
#simduino.cont()
#simduino.wait()
angr_target.cont()
