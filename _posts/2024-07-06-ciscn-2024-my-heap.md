...
Finally, I understand how `malloc(0x4f0);` in the `add` function plays a role to leak libc. Basically, this program allows us to allocate a chunk of whatever size. If we could free a chunk to **unsorted_bin**, then know where `main_arena` is, which helps us calculate libc base address because there is a fixed offset in between. When a heap is initilized, unsorted_bin will be done the same way as well:
```c
...
This is why we could see the first freed chunk in the unsorted list, whose `fd` and `bk` pointers both point to the address of `main_arena` plus an offset.
